...
적용 스크립트는 Debian/Ubuntu 계열처럼 `apt-get`이 있는 환경에서 `zsh`, `git`, `curl`, `ca-certificates`를 설치한다. `btop`, `fastfetch`는 apt 저장소에 있을 때만 설치한다. 이후 `oh-my-zsh`와 `zsh-autosuggestions`, `zsh-syntax-highlighting`을 설치한다.
